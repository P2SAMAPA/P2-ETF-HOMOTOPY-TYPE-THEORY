...
from homotopy_type import homotopy_type_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    for ticker in ret_window.columns:
        ...
        s = homotopy_type_score(
            ret_window[ticker].values,
            macro_window,
            hit_levels=config.HIGHER_INDUCTIVE_LEVELS,
            univalence_threshold=config.UNIVALENCE_THRESHOLD
        )
    ...
    top_etfs = [{"ticker": t, "hott_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/hott_{timestamp}.json"
