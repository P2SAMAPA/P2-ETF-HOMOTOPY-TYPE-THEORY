...
st.set_page_config(page_title="Homotopy Type Theory", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🌀 Homotopy Type Theory</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Dependent type theory | Path equivalence | Higher inductive types | Univalence axiom</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 HoTT")
...
st.sidebar.markdown(f"**HIT levels:** {config.HIGHER_INDUCTIVE_LEVELS} | **Univalence threshold:** {config.UNIVALENCE_THRESHOLD}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'hott_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Homotopy level: {etf['hott_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Homotopy Level"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Homotopy Level", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🌀 Top ETFs by Homotopy Type Level (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Homotopy Type Theory (HoTT)** treats portfolios as terms in a dependent type theory.
        - **Path equivalence** captures continuous deformations between market states.
        - **Higher inductive types** encode trading constraints.
        - The **univalence axiom** allows transport of strategies across isomorphic structures.
        - The score is the homotopy level (truncation level) – higher = more structure.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the homotopy levels.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Homotopy Type Theory | Dependent types for portfolio structure")
